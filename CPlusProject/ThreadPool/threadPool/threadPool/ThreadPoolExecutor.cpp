...
CWorker::CWorker(CThreadPoolExecutor * pThreadPool, Runnable * pFirstTask) : 
...
void CWorker::Run()
{
	...
	while(m_bRun)
	{
		...
		if(NULL == pTask)
		{
			...
			{
				auto itr = m_pThreadPool->m_ThreadPool.find(this);
			...
			{
				auto itr = m_pThreadPool->m_TrashThread.begin();
